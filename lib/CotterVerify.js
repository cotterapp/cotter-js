...
class CotterVerify {
    constructor(config, tokenHandler) {
        ...
        if (this.config.CotterBaseURL)
            _1.default.JSURL = this.config.CotterBaseURL;
    ...
    showForm() {
        ...
        helper_1.challengeFromVerifier(this.verifier).then((challenge) => {
            var path = `${_1.default.JSURL}/signup?challenge=${challenge}&type=${this.config.Type}&domain=${this.config.Domain}&api_key=${this.config.ApiKeyID}&redirect_url=${this.config.RedirectURL}&state=${this.state}&id=${this.cID}`;
    ...
    submitAuthorizationCode(payload) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            fetch(`${_1.default.BackendURL}/verify/get_identity?oauth_token=true`, {
                method: "POST",
                headers: {
                ...
                return __awaiter(this, void 0, void 0, function* () {
                    ...
                    data.user = resp.user;
                    var user = new User_1.default(resp.user);
                    user.store();
                    if (self.tokenHander) {
    ...
    static sendPost(data, iframeID) {
        ...
        if (isIFrame(ifrm) && ifrm.contentWindow) {
            ifrm.contentWindow.postMessage(JSON.stringify(data), _1.default.JSURL);
        }
    }
}
