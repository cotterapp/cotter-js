            ...
            switch (data.callbackName) {
                ...
                case cID + "ON_BEGIN":
                    ...
                    if (!!_this.config.OnBegin) {
                        // if OnBegin is async
                        if (_this.config.OnBegin.constructor.name === "AsyncFunction" ||
                            _this.config.OnBegin.toString().includes("async")) {
                            _this.config.OnBegin(data.payload).then(function (err) {
                                if (!err)
                            ...
                            var err = _this.config.OnBegin(data.payload);
                            if (!err)
                                _this.continue(data.payload, _this.cotterIframeID);
                            else
                                _this.StopSubmissionWithError(err, _this.cotterIframeID);
