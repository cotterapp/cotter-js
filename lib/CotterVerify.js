...
class Cotter {
    ...
    showForm() {
        ...
        helper_1.challengeFromVerifier(this.verifier).then((challenge) => {
            var path = `${enum_1.default.CotterBaseURL}/signup?challenge=${challenge}&type=${this.config.Type}&domain=${this.config.Domain}&api_key=${this.config.ApiKeyID}&redirect_url=${this.config.RedirectURL}&state=${this.state}&id=${this.config.ContainerID}`;
