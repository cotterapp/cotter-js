...
var MagicLink = /** @class */ (function (_super) {
    ...
    MagicLink.prototype.showForm = function () {
        ...
        helper_1.challengeFromVerifier(this.verifier).then(function (challenge) {
            var path = enum_1.default.JSURL + "/login?auth_method=MAGIC_LINK&code_challenge=" + challenge + "&type=" + _this.config.Type + "&domain=" + _this.config.Domain + "&api_key=" + _this.config.ApiKeyID + "&redirect_url=" + encodeURIComponent(_this.config.RedirectURL) + "&state=" + _this.state + "&id=" + _this.cID;
