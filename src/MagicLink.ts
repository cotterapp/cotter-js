...
class MagicLink extends CotterVerify {
  ...
  showForm() {
    ...
    if(!this.config.RedirectURL) this.config.RedirectURL = window.location.href
    this.config.SkipRedirectURL = true

    challengeFromVerifier(this.verifier).then((challenge) => {
      var path = `${CotterEnum.JSURL}/login?auth_method=MAGIC_LINK&code_challenge=${challenge}&type=${this.config.Type}&domain=${this.config.Domain}&api_key=${this.config.ApiKeyID}&redirect_url=${encodeURIComponent(this.config.RedirectURL)}&state=${this.state}&id=${this.cID}`;
