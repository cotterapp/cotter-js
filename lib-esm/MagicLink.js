...
var MagicLink = /** @class */ (function (_super) {
    ...
    MagicLink.prototype.showForm = function () {
        ...
        challengeFromVerifier(this.verifier).then(function (challenge) {
            var path = CotterEnum.JSURL + "/login?auth_method=MAGIC_LINK&code_challenge=" + challenge + "&type=" + _this.config.Type + "&domain=" + _this.config.Domain + "&api_key=" + _this.config.ApiKeyID + "&redirect_url=" + encodeURIComponent(_this.config.RedirectURL) + "&state=" + _this.state + "&id=" + _this.cID + "&redirect_link=" + _this.config.RedirectMagicLink;
